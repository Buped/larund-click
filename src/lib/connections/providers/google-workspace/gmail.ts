  ...
  },
  {
    ...
    async run(args, secrets) {
      ...
      return googleResult(async () => {
        ...
        return {
          success: true,
          output: `Gmail piszkozat létrehozva (${to} – "${subject}"). Read-back: ${verified ? 'megerősítve' : 'nem megerősíthető'}.`,
          details: { draftId, messageId: created.message?.id, to, subject, verified },
        };
      });
    },
  },
